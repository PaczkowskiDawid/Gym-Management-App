...
exports.login = async(req,res)=>{
    try{
        ...
        if(gym && await bcrypt.compare(password, gym.password)){
            ...
            const token = jwt.sign({gym_id:gym.id},process.env.JWT_SecretKey);

            res.cookie("cookie_token", token, cookieOptions)

            res.json({ message: 'Logged in succesfully' , succes: "true", gym,token});
...
exports.sendOtp =  async (req,res) => {
    try {
        ...
        if(gym){
            ...
            gym.resetPasswordExpires = Date.now() + 3600000; // one hour expiry date 

            await gym.save();

            //for email Sending
            const mailOptions = {
                from: 'dawideric2003@gmail.com',
                to: email,
