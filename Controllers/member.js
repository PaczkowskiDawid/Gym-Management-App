...
exports.searchMember = async (req,res)=> {
    try{
        ...
        res.status(200).json ({
            message:member.length?"Fetched Members Successfully":"No any Member Registered yet",
            members:member,
    ...
    try{
        ...
        const member = await Member.find({gym:req.gym._id,
            createdAt:{
                ...
                $lte: endOfMonth    //Less than or equal to the las day of the month
            }
        }).sort({createdAt:-1});
        res.status(200).json ({
            message:member.length?"Fetched Members Successfully":"No any Member Registered yet",
...
exports.expiringWithIn4To7Days = async (req,res) => {
    try{
        ...
        const member = await Member.find ({ gym:req.gym._id,
            nextBillDate: {
                $gte : next4Days,           //Greater than or equal to 4days later from today
...
exports.updateMemberPlan = async(req,res) => {
    try{
        ...
        if(memberShip){
            ...
            return res.statu(409).json ({error:"No such Membership are there"})
        }
